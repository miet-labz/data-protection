...
fn enter_p_q() -> (u32, u32){
    ...
    let vec = buf.split_whitespace().map(|n| n.parse::<u32>().expect("Wrong input!")).collect::<Vec<u32>>();
    (vec[0], vec[1])
}
...
fn enter_encr_str() -> Vec<u32>{
    ...
    let mut buf = String::new();
    io::stdin().read_line(&mut buf).expect("Failed to read line");
    let out = buf.split_whitespace().map(|n| n.parse::<u32>().expect("Wrong input!")).collect::<Vec<u32>>();
    out
}
...
fn main() {
    if option().expect("Wrong input"){
        ...
        let d = dn.0;
        let n = dn.1;
        let s = encr_s.iter().map(|c| (pow_by_mod(*c as u32, d, n) as char)).collect::<String>();
        println!("{}", s);
    }
